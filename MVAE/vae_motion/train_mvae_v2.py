...
from vae_motion.custom_models import CustomPoseMixtureVAE
EPS = 1e-6
...
def main():
    ...
    args = SimpleNamespace(
        ...
        mocap_file=os.path.join(env_path, "AIST_music_mvae.npz"),
        norm_mode="zscore",
        # norm_mode="minmax",
        latent_size=32,
        num_embeddings=12,
    ...
    args.final_lr = 1e-7
    print("loading data: {}".format(args.mocap_file))
    ...
    if args.norm_mode == "minmax":
        ...
        mocap_data = 2 * (mocap_data - min) / denominator - 1
    ...
    save_path = os.path.join(dirname, 'model_files', os.path.split(args.mocap_file)[-1][:-4])
    ...
    for ep in range(1, args.num_epochs + 1):
        ...
        for num_mini_batch, indices in enumerate(sampler):
            ...
            history[:, : args.num_condition_frames].copy_(mocap_data[condition_range]) ## initialize history using groudn truth data for training
            ...
            for offset in range(args.num_steps_per_rollout):
                ...
                next_frame = vae_output[:, 0] if use_student else ground_truth[:, 0] ### upate history, using mocap data if no scheduled learning
                ...
                # print("ep_recon_loss: {}".format(float(recon_loss) / args.num_steps_per_rollout))
        ...
        # print("avg_ep_recon_loss: {}".format(avg_ep_recon_loss))
        # print("avg_ep_kl_loss: {}".format(avg_ep_kl_loss))
        # print("num_mini_batch: {}".format(num_mini_batch))
        logger.log_stats(
            {
                "epoch": ep,
        ...
        ### save every epoch
        torch.save(copy.deepcopy(pose_vae).cpu(), os.path.join(save_path, pose_vae_path[:-3] + '_' + str('zscore') + '.pt'))
        # torch.save(copy.deepcopy(pose_vae).cpu(), os.path.join(save_path, pose_vae_path))
