...
def get_subsequent_mask(seq, sliding_windown_size):
    ...
    #mask = torch.triu(
    #   torch.ones((seq_len, seq_len), device=seq.device, dtype=torch.bool), diagonal=-sliding_windown_size)
...
class Model(nn.Module):
    def __init__(self, encoder, decoder, glow,
                 condition_step=10, sliding_windown_size=100, lambda_v=0.01, device=None):
        super().__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.glow = glow
        self.linear = nn.Linear(decoder.hidden_size + encoder.d_model, 375)
    ...
    def forward(self, src_seq, src_pos, tgt_seq,  hidden, dec_output, out_seq, epoch_i):
        ...
        mask = torch.cat([prediction_mask, groundtruth_mask], 1).view(-1)[:seq_len]  # for curriculum learning

        for i in range(seq_len):
            dec_input = tgt_seq[:, i] if mask[i] == 1 else dec_output.detach()  # curriculum learning
