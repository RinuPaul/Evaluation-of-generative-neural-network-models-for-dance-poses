...
import numpy as np
import scipy.linalg
from thops import sum_s,mean_m,split_feature,cat_feature,timesteps_fn
...
class BatchNorm(nn.Module):
    ...
    def _get_moments(self, x):
        mean = mean_m(x.clone(), dim=[0, 2, 3], keepdim=True).detach()
        var = mean_m((x.clone() - mean) ** 2, dim=[0, 2, 3], keepdim=True).detach()
...
class _ActNorm(nn.Module):
    ...
    def initialize_parameters(self, input):
        ...
        with torch.no_grad():
            bias = mean_m(input.clone(), dim=[0, 2], keepdim=True) * -1.0
            vars = mean_m((input.clone() + bias) ** 2, dim=[0, 2], keepdim=True)
    ...
    def _scale(self, input, logdet=None, reverse=False):
        ...
        if logdet is not None:
            ...
            """
            dlogdet = sum_s(logs) * timesteps_fn(input)
            if reverse:
                dlogdet *= -1
...
class InvertibleConv1x1(nn.Module):
    ...
    def get_weight(self, input, reverse):
        ...
        if not self.LU:
            ...
            dlogdet = sum_s(self.log_s) * timesteps_fn(input)
            if not reverse:
...
class GaussianDiag:
    ...
    def logp(self,x):
        ...
        return sum_s(likelihood, dim=[1, 2])
