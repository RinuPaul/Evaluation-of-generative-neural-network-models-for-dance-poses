...
if __name__ == '__main__':
    ...
    if opt.continue_train:
        ...
        if opt.load_weights_only:
            ...
            trainer = Trainer.from_argparse_args(args, gpus = 1,auto_select_gpus=True,max_epochs=150,logger=logger, default_root_dir=default_save_path, plugins=plugins, callbacks=callbacks)
        else:
            trainer = Trainer.from_argparse_args(args, gpus =1,auto_select_gpus=True,max_epochs=150,logger=logger, default_root_dir=default_save_path, resume_from_checkpoint=latest_file, plugins=plugins, callbacks=callbacks)
    else:
        trainer = Trainer.from_argparse_args(args, gpus =1,auto_select_gpus=True,max_epochs=150,logger=logger, default_root_dir=default_save_path, plugins=plugins, callbacks=callbacks)
