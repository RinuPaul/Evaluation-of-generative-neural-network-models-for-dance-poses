...
class BaseOptions:
    def __init__(self):
        ...
        parser.add_argument('-d', '--data_dir', type=str, default='data/motion')
        parser.add_argument('--hparams_file', type=str, default='training/hparams/dance_combined/transflower_expmap.yaml')
        parser.add_argument('--dataset_name', type=str, default="multimodal")
        parser.add_argument('--base_filenames_file', type=str, default="base_filenames.txt")
    ...
    def gather_options(self, parse_args=None):
        ...
        if opt.hparams_file is not None:
            if hparams_file.endswith(".json"):
                ...
                hparams_json = yaml.load(open(hparams_file),Loader=yaml.FullLoader)
